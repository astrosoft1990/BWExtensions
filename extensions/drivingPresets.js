...
(function() {
    ...
    try {
        ...
        if (__win.drivingPresets) return;
        __win.drivingPresets = true;
    ...
    function PresetHandler() {
        ...
        function loadPresets() {
            const savedPresets =  ext_getValue(STORE_KEY, [])
            return [].concat(PRESETS).map(item => { item.static = true; return item; }).concat(savedPresets);
        }
        function savePresets() {
            ext_setValue(STORE_KEY, presets.filter(function(preset) {
